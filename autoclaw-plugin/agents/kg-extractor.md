...
2. KG_EXISTING: semantic facts already in the knowledge graph (from `chartcode export`)
...
Produce valid JSON and pipe it to `chartcode reconcile`:
...
Then run:
```bash
echo '<your_json>' | chartcode reconcile
```
